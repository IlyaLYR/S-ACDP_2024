...
public class MainForm extends JFrame {
    ...
    public MainForm() {
        this.setTitle("№1. Лабиринт Минотавра");
        ...
        cleanButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                game.cleanField();
                resultLabel.setText("Минимальное количество клеток: " + 0);
                updateView();
            }
        });
    ...
    private JMenuBar createMenuBar() {
        ...
        menuGame.add(createMenuItem("Загрузить поле", "ctrl+J", null, e -> {
            FileNameExtensionFilter filter = new FileNameExtensionFilter("Text Files (*.txt)", "txt");
            fileChooser.setFileFilter(filter);
            fileChooser.setCurrentDirectory(new File(System.getProperty("user.dir")));
            fileChooser.showDialog(null, "Выбрать поле");

            if (fileChooser.getSelectedFile() != null) {
                try {
                    game.readFromTXT(fileChooser.getSelectedFile().getAbsolutePath());
                    params.setRowCount(game.getRowCount());
                    params.setColCount(game.getColCount());
